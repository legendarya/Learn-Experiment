...
class UrlController extends PageTemplate{
	...
	function muestraApartado($apartado){
		...
		$curso=$apartado;
		while($curso->padre->padre) $curso=$curso->padre;
		...
		return new vDiv(array(
				...
				($this->esResponsable($apartado)?new vDiv(array(
					...
					($this->esResponsable($curso)?new vLink(base_url().'apartados/reorganizar/'.$curso->id,'Reorganizar apartados','','buttonLink'):''),
					new vLink(base_url().'ejercicio/crear_abierto//'.$apartado->id,'Crear ejercicio','','buttonLink'),
					new vLink(base_url().'ejercicio/enlaza/'.$apartado->id,'Enlaza ejercicio','','buttonLink')
				)):''),
