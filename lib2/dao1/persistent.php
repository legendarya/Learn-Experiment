...
class Persistent{
	...
	function __construct($name,$condition=false){
		...
		if ($condition){
			...
			else $query='select * from '.$this->entity->name.' where id='.$condition;
	...
	function loadData($row){
		foreach($this->entity->fields as $field)
			if($field instanceof DateField)
				...
				if($row[$field->name]){
					$this->fields[$field->name]=mb_unserialize(stripslashes($row[$field->name]));
					
					if($this->fields[$field->name]===false) {
						print_r(mb_unserialize(stripslashes($row[$field->name])));
						throw new Exception('Field "'.$field->name.'" in row '.$row['id'].' of entity "'.$this->entity->name.
							'" cannot be unserialized ("'.$row[$field->name].'")');
					}
				}
				...
				else $this->fields[$field->name]=stripslashes($row[$field->name]);
		$this->id=$row['id'];
	}
	...
	private function prepareFields(){
		$values=array();
		foreach($this->fields as $name=>$field) if(isset($this->entity->fields[$name]))
			switch(get_class($this->entity->fields[$name])){
				case 'UniversalTextField': 
					if($field[0]){
						if($this->fields[$name][1]!==null)
							mysql_query('update texto set texto="'.addslashes($field[1]).'" where id='.$field[0]);
					}
					else{
						$resource=mysql_query('select max(id) from texto');
						$result = mysql_fetch_row($resource);
						$id=$result[0]+1;
						mysql_query('insert into texto (id,idioma,texto) values ('.$id.',"'.$this->language.'","'.addslashes($field[1]).'")');
						$values[$name]=$id;
					}
				break;
				case 'DateField': 
				...
				break;
				case 'StructField': 
					$values[$name]='"'.addslashes(serialize($field)).'"';
				break;
				case 'ReferenceField':
					...
					else $values[$name]='"'.addslashes($field).'"';
	...
	function save($show_query=false){
		...
		if ($this->id){
			...
			if(count($values)){
				$query='update '.$this->entity->name.' set '.implode(',',$values).' where id='.$this->id;
