...
class persistentList implements Iterator{
	...
	function update($set){
		...
		if(count($this->order)){
			...
			foreach($this->order as $order)
				if($order=='RAND()')
					$order_info[]='RAND()';
				else $order_info[]=(($this->listSide and $this->listSide->entity->name)?
	...
	function getValue($value){
		...
		$result=query('select '.$value.' from '.$this->getQueryTables().$this->getQueryWhere().$group.$order.$limit);
			
		if(isset($result[0]) and isset($result[0][0])) return $result[0][0];
		else return null;
	}
	...
	function orderBy($field,$asc=true){
		if(!$this->table and $field!='id' and $field!='RAND()' and !isset($this->listSide->entity->fields[$field])) 
	...
	function __call($name,$arguments){
		...
		elseif(substr($name,-5)=='IsNot'){
			...
			if(!isset($entity->fields[$field]) and $field!='id') throw new Exception('There is no field "'.$field.'" on entity "'.$entity->name.'"');
		...
		elseif(substr($name,-15)=='LessOrEqualThan'){
			$field=substr($name,0,strlen($name)-15);
			
			if($this->table) $entity=entity::$entities[$this->table];
			else $entity=$this->listSide->entity;
			if(!isset($entity->fields[$field])) throw new Exception('There is no field "'.$field.'" on entity "'.$entity->name.'"');
		
			if($arguments[0] instanceof Persistent) return $this->where(new LessOrEqualThanCondition($field,$arguments[0]->id));
			else return $this->where(new LessOrEqualThanCondition($field,$arguments[0]));
		}
		elseif(substr($name,-15)=='MoreOrEqualThan'){
			$field=substr($name,0,strlen($name)-15);
			...
			if($arguments[0] instanceof Persistent) return $this->where(new MoreOrEqualThanCondition($field,$arguments[0]->id));
			else return $this->where(new MoreOrEqualThanCondition($field,$arguments[0]));
		}
