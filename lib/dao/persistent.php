...
class Persistent{
	...
	function loadData($row){
		foreach($this->entity->fields as $field)
			if($field instanceof DateField)
				...
				if($row[$field->name]){
					$this->fields[$field->name]=unserialize(stripSlashes(base64_decode($row[$field->name])));
				}
	...
	private function prepareFields(){
		...
		foreach($this->fields as $name=>$field) if(!$this->fields[$name] instanceof PersistentList)
			switch(get_class($this->entity->fields[$name])){
				...
				break;
				case 'StructField': 
					$values[$name]='"'.base64_encode(addSlashes(serialize($field))).'"';
				break;
				case 'ReferenceField':
	...
	private function getOrderList(){
		foreach($this->entity->sets as $name=>$set){
			foreach($set->sides as $side)
				if($side->order and $name!=$side->property) return array($name,$side);
		}
	}
	
	function moveForward(){
		list($name,$side)=$this->getOrderList();
	...
	function insertBefore($apartado){
		list($name,$side)=$this->getOrderList();
		$order=$side->order;
		query('update '.$this->entity->name.' set '.$this->entity->name.'.'.$side->order.'='.$this->entity->name.
			'.'.$order.'-1 where '.
			$this->entity->name.'.'.$order.'>'.$this->fields[$order].' and '.$this->entity->name.'.'.
			$name.'='.$this->$name->id);
			
		query('update '.$this->entity->name.' set '.$this->entity->name.'.'.$side->order.'='.$this->entity->name.
			'.'.$order.'+1 where '.
			$this->entity->name.'.'.$order.'>='.$apartado->$order.' and '.$this->entity->name.'.'.
			$name.'='.$apartado->$name->id);
		
		// Ponemos este sustituyendo al apartado
		query('update '.$this->entity->name.' set '.$this->entity->name.'.'.$order.'='.$apartado->$order.','.
			$this->entity->name.'.'.$name.'='.$apartado->$name->id.' where id='.$this->id);
